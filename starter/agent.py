...
class _SessionState:
    __slots__ = ("profile", "category", "constraints", "no_pref", "card_drained")
...
class Agent:
    ...
    def _respond(self, session_id: str, user_message: str, turn: int, top_k: int) -> dict:
        ...
        message = (
            "Here are the closest matches so far — anything else that matters to you?"
            if ask
            else "Got it — these are my best matches for everything you've told me."
        )
        return {
            "message": message,
            "ask_attribute": ask,
    ...
    def _rank(self, state: _SessionState, top_k: int) -> list[str]:
        ...
        return [pid for _, pid in reranked[:top_k]]
