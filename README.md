...
1. **Extract** — a deterministic parser handles the protocol's message
   templates; anything off-template goes to an optional Claude extraction
   layer that emits the same slot structure (see *LLM layer* below).
2. **Bank** — constraints accumulate in session state. "No preference for X"
   locks an attribute; when the card is exhausted we know the banked set is
   the *complete* intent card, so products whose card matches it exactly get
   boosted over superset lookalikes.
3. **Retrieve & rank** — intersect index hits; score by weighted constraint
   ...
   never a hard filter) + profile-tag and popularity tie-breaks.
...
python3 -m pytest tests/               # unit tests (parser, index, agent, LLM fallback)
